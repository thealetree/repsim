...
export const SCENARIOS: ScenarioDef[] = [
  ...
  // immunity. A virus is seeded in one random organism at start.
  {
    ...
    preamble: [
      ...
      'Each time a virus jumps to a new host, it copies itself imperfectly. Its spread, damage, and lethality each wander a little from the parent\'s, and occasionally the mutation is large enough to split off a genuinely new strain. So the numbers you read off any infected organism aren\'t locked by the Virus panel sliders — those are just global multipliers. The strain itself is always drifting, and across the run you\'ll see the dominant lineages shift in response to whatever\'s working.',
      'You\'ll know an infected rep when you see one: segments matching the virus\'s target color go dark and muted (color corruption), and some organisms move with a twitchy, wobbling gait as joint strength fluctuates. The top right panel counts active Strains and Infected organisms; clicking an infected rep shows its strain and whether the infection rolled lethal or merely weakening.',
